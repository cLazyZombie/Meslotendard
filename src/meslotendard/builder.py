...
def derive_monospace_advance(font: TTFont) -> int:
    ...
    if not cmap:
        raise ValueError("Meslo cmap을 읽을 수 없습니다.")
    ...
    if len(widths) != 1:
        raise ValueError(f"Meslo ASCII advance가 단일 값이 아닙니다: {sorted(widths)}")
    return widths.pop()
...
def scale_latin_horizontally(
    ...
    *,
    ...
    """Meslo 윤곽을 조정하고 지정한 advance 안에 중앙 정렬한다."""
...
def update_metadata(font: TTFont, variant: Variant, project_version: str) -> None:
    """패밀리·스타일·버전·라이선스 메타데이터를 Meslotendard로 고정한다."""
    ...
    values = {
        0: (
            "Meslotendard Project Authors; Monatendard Project Authors; Meslo LG "
            "copyright 2009, 2010, 2013 Andre Berg; Pretendard copyright 2021 "
            "Kil Hyung-jin; Source copyright 2014-2021 Adobe; Inter copyright "
            "2016 The Inter Project Authors; M PLUS copyright 2021 The M+ FONTS "
            "Project Authors; "
            "Nerd Fonts contributors and upstream glyph authors."
        ),
        1: FAMILY_NAME,
        ...
        6: postscript_name,
        13: (
            "Meslotendard is a modified font derived from Meslo LG (Apache-2.0), "
            "Pretendard (OFL-1.1), and Nerd Fonts glyph sources under their "
            "respective licenses, including MIT, OFL-1.1, Apache-2.0, and "
            "CC-BY-4.0. The combined font is distributed under the SIL Open "
            "Font License 1.1. "
            "See the Meslotendard repository for notices."
        ),
...
def build_font(
    ...
    *,
    ...
    latin_path = MESLO_DIR / variant.latin_filename
    connecting_path = (
        MESLO_DIR / variant.upright_latin_filename if variant.is_italic else latin_path
    )
    cjk_path = PRETENDARD_DIR / variant.cjk_filename
    for path in (latin_path, connecting_path, cjk_path):
        if not path.exists():
            raise FileNotFoundError(
                f"원본 파일이 없습니다. 먼저 `meslotendard fetch`를 실행하세요: {path}"
            )
